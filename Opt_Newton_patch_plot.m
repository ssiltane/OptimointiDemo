...
% Define contour levels for contour plots
contourvec = linspace(min(minimfun(:)),max(minimfun(:)),Ncontours);

% Find minimizer
minimindex = min(find(minimfun==funMIN));

% Save the function to file
save data/minimfun minimfun funMIN funMAX contourvec minimindex X Y t1MAX t2MAX Ncontours
...
while (iii<Niter)&(gradnorm>gradtol)
    ...
    % Save to file
    filename = ['frames_Newton_patch/OptFrameNewton_',num2str(imcounter),'.png'];
...
% Save to file
filename = ['frames_Newton_patch/OptFrameNewton_0.png'];
imwrite(uint8(im3),filename,'png');
imcounter = imcounter+1;



%% Draw very first image without initial guess
% Plot the function to be minimized
figure(1)
clf
...
% Save to file
filename = ['frames_Newton_patch/OptFrameNewton_00.png'];
imwrite(uint8(im3),filename,'png');
